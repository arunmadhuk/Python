...
class MainWindow(QMainWindow):
    ...
    def createGroupSet(self):
        if cmds.objExists("group_SET"):
            return
        else:
            self.groupSet = cmds.sets(name="group_SET", empty=True)

    def deleteExistingGroupSet(self):
        if cmds.objExists("group_SET"):
            childSets = cmds.sets("group_SET", q=True)
            for child in childSets:
                print 'child set deleted ', child
                cmds.delete(child)

            # cmds.delete("group_SET")
            print"group_SET deleted"
        else:
            return
    ...
    def addSelectedObject(self):
        ...
        if self.setName in self.classifiedObjects:
            ...
            self.objSelected = cmds.ls(selection=True)
            for i in range(self.objectListWidget.count()):
                obj = self.objectListWidget.item(i).text()
                # print obj
            ...
            if self.objSelected:
                ...
                for obj in self.objSelected:
                    # print obj
                    if obj not in objectsList:
                        objectsList.append(obj)
                        # print objectsList
                        self.objectListWidget.addItem(QListWidgetItem(obj))

                self.classifiedObjects[self.setName] = objectsList
    ...
    def check_invalid_sets(self):
        allSets = cmds.listSets(allSets=True)
        invalidSets = []

        for setName in allSets:
            if setName in self.classifiers:
                matches = [match for match in allSets if setName in match]
                # print matches
                if len(matches) >1:
                    for match in matches:
                        invalidSets.append(match)


        # print "invalid sets ----- ", invalidSets

    def load_existing_sets(self):
        # self.check_invalid_sets()
        allSets = cmds.listSets(allSets=True)
        deleteSets = []

        for setName in allSets:

            if "group_SET" in allSets:
                childSets = cmds.sets("group_SET", q=True)
                for child in childSets:
                    if child not in self.classifiers:
                        print 'Invalid child set name ', child
                        cmds.delete(child)
            ...
            #
            #     for match in matches:
            #         deleteSets.append(match)

            if setName in self.classifiers and "group_SET" in allSets:
                objs = cmds.sets(setName, query=True)
                objList = []
                for obj in objs:
                    objList.append(str(obj))
                    # print set, " --- ", obj
                self.classifiedObjects[setName] = objList
            if setName in self.classifiers and "group_SET" not in allSets:
                deleteSets.append(setName)

        # print "deleteSets --", deleteSets

        if deleteSets > 0:

            for sets in list(dict.fromkeys(deleteSets)):
                if sets in self.classifiers and cmds.objExists(sets):
                    cmds.delete(sets)

            lambda: self.load_existing_sets()

    def createSets(self):
        allSets = cmds.listSets(allSets=True)
        keys = self.classifiedObjects.keys()

        self.deleteExistingGroupSet()

        for key in keys:
            matches = [match for match in allSets if key in match]
            for match in matches:
                # print cmds.objExists(match)
                if cmds.objExists(match):
                    cmds.delete(match)

        self.createGroupSet()

        for key, values in self.classifiedObjects.items():
            # print "Key --- ", key
            # print "value --- ", values
            selectedObjects = []
            if len(values) != 0:
                ...
                if selectedObjects:
                    ...
                    # print "objects --- ", selectedObjects
                    # print self.groupSet

                    newSet = cmds.sets(name=key)
                    cmds.sets(newSet, forceElement="group_SET")
                    ...
                    for obj in selectedObjects:
                        objList.append(obj)
                        # print set, " --- ", obj
                    self.classifiedObjects[key] = objList
